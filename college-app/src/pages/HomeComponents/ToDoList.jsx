...
const ToDo = () => {
    ...
    const addItem = () => {
        ...
        if (list.map(item => item.toLowerCase()).includes(trimmedItem.toLowerCase())) {
    ...
    const removeCompletedItem = (itemToRemove) => {
        setCompleted((prevList) => prevList.filter(item => item !== itemToRemove))
    };
            ...
            <div className="current-tasks-section">
                ...
                {list.length === 0 ? (
                    ...
                    <ul className="todo-items">
                        {list.map((item, index) => (
                            <li key={item}>
                                <button className="complete-button" onClick={() => completeItem(item)}>
                                    &#x2713;
                                </button>
                                <span>{item}</span>
                ...
                {completed.length === 0 ? (
                    ...
                    <ul className="completed-items">
                        {completed.map((item, index) => (
                            <li key={item}>
                                <span>{item}</span>
                                <button onClick={() => removeCompletedItem(item)} className="remove-button">
                                    &times;
                                </button>
                            </li>
                        ))}
